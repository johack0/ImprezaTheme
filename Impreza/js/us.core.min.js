...
self.$container.usMod('pos',self.pos=pos);if(self.pos==='static'){self.changeSticky(!1)}},_setBg:function(bg){const self=this;if(bg!=self.bg){self.$container.usMod('bg',self.bg=bg)}},_setShadow:function(shadow){const self=this;if(shadow!=self.shadow){self.$container.usMod('shadow',self.shadow=shadow)}},_isVerticalScrollable:function(){const self=this;if(!self.isVertical()){return}
if((self.currentStateIs('default')||self.currentStateIs('laptops'))&&self.isFixed()){self.$container.addClass('scrollable');var headerHeight=self.getHeight(),canvasHeight=parseInt($us.canvas.winHeight),documentHeight=parseInt($us.$document.height());self.$container.removeClass('scrollable');if((headerHeight/canvasHeight)>1.05){self.swichVerticalScrollable(!0)}else if(self._states.vertical_scrollable){self.swichVerticalScrollable(!1)}
if((headerHeight/documentHeight)>1.05){self.$container.css({position:'absolute',top:0})}}else if(self._states.vertical_scrollable){self.swichVerticalScrollable(!1)}},swichVerticalScrollable:function(state){const self=this;self.$container.toggleClass('scrollable',self._states.vertical_scrollable=!!state);if(!self._states.vertical_scrollable){self.$container.resetInlineCSS('position','top','bottom');delete self._headerScrollRange}},changeSticky:function(state){const self=this;self._states.sticky=!!state;var currentHeight=self.getCurrentHeight(!0),resetCss=['position','top','bottom'];if($us.canvas.hasStickyFirstSection()&&self.getHeaderInitialPos()=='bottom'&&!self.stickyAutoHideEnabled()){resetCss=resetCss.filter((value)=>{return value!=='top'})}
...
var headerAttachedFirstSection=['bottom','below'].includes(self.getHeaderInitialPos());if(self.isHorizontal()&&(headerAbovePosition||(headerAttachedFirstSection&&(self.currentStateIs('tablets')||self.currentStateIs('mobiles')))||!headerAttachedFirstSection)){if(self.stickyEnabled()){var scrollBreakpoint=parseInt(((self.settings[self.state]||{}).options||{}).scroll_breakpoint)||100,isSticky=Math.ceil(scrollTop)>=scrollBreakpoint;if(isSticky!=self.isPinned()){self.changeSticky(self._states.sticky!==isSticky)}}
if(self.isPinned()&&!_window.scrollY){self.changeSticky(!1)}}
if(self.isHorizontal()&&headerAttachedFirstSection&&!headerAbovePosition&&(self.currentStateIs('default')||self.currentStateIs('laptops'))){var top=($us.canvas.getHeightFirstSection()+self.adminBarHeight);if(self.getHeaderInitialPos()=='bottom'){top-=self.getInitHeight()}
if(self.stickyEnabled()){var isSticky=_window.scrollY>=top;if(isSticky!=self.isPinned()){self.changeSticky(isSticky)}}
if(!self.isPinned()&&top!=self.getOffsetTop()){self.$container.css('--heading-below-first-section-top',`${top}px`)}}
...
$ush.timeout(()=>{self.clearChars(part,nextValue)},curDuration+Math.floor(self.opts.delay/3))},startDelay)},clearChars:function(part,text){const self=this;part.$part.html(text.replace(REGEXP_SPACE,self.spaceChar));$ush.timeout(()=>self.charsNext(part),self.opts.delay)}});$.fn.usItext=function(){return this.each(function(){$(this).data('usItext',new usItext(this))})};$(()=>$('.w-itext').usItext())}(jQuery);!function($){"use strict";$us.WLogin=function(container,options){this.init(container,options)};$us.WLogin.prototype={init:function(container,options){this.$container=$(container);if(this.$container.data('loginInit')==1){return}
...
self.xhr=$.ajax({type:'post',url:ajaxUrl,dataType:'html',cache:!1,data:ajaxData,success:(htmlContent)=>{if(listFiltersApplied){if(self.$container.hasClass('type_masonry')){self.$list.isotope('remove',$('.w-grid-item',self.$list)).isotope('layout')}
self.$list.html('');self.$none.addClass('hidden')}
const $htmlContent=$(htmlContent);$('.w-grid.us_post_carousel, .w-grid.us_product_carousel',$htmlContent).remove();if(self.paginationType=='numbered_ajax'){const numPagination=$ush.toString($('nav.pagination.navigation',$htmlContent).html());self.$pagination.toggleClass('hidden',!numPagination).html(numPagination);self.$list.html('')}
var $listJson=$('.w-grid-list-json:first',$htmlContent);if($listJson.is('[onclick]')){if($listJson[0].onclick===null){$listJson[0].onclick=new Function($listJson.attr('onclick'))}
$.extend(!0,self.data,$listJson[0].onclick()||{})}
var $items=$htmlContent.find('.w-grid-list').first().children();$ush.timeout(()=>{const data={postListUid:self.uid,listFilterUid:self.listFilterUid,listFiltersApplied:listFiltersApplied,};$us.$document.trigger('usPostList.itemsLoaded',[$items,data])},50);if(!$items.length){if(!self.$none.length){self.$none=$('.w-grid-none:first',$htmlContent);if(!self.$none.length){self.$none=$htmlContent.filter('.w-grid-none:first')}
...
if(self.paginationType=='numbered'){const $pagination=$('nav.pagination',$htmlContent);if($pagination.length&&!self.$pagination.length){self.$list.after($pagination.prop('outerHTML'));self.$pagination=self.$list.next('nav.pagination')}
...
if(!animated){this.$tabsBar.css(css)}else{this.$tabsBar.performCSSTransition(css,this.options.duration,null,this.options.easing)}},openSection:function(index){const self=this;if(this.sections[index]===_undefined){return}
if(this.curLayout==='hor'){this.$sections.removeClass('active').css('display','none');this.sections[index].stop(!0,!0).fadeIn(this.options.duration,function(){$(this).addClass('active');self.startTabComponents(index)})}else if(this.curLayout==='accordion'){if(this.contents[this.active[0]]!==_undefined){this.contents[this.active[0]].css('display','block').stop(!0,!1).slideUp(this.options.duration,()=>self.startTabComponents(index))}
...
if(fx.end!==newTop){$us.$htmlBody.stop(!0,!1).animate({scrollTop:newTop},$.extend(animateOptions,{easing:$us.getAnimationName('easeOutExpo')}))}}.bind(this)};$us.$htmlBody.stop(!0,!1).animate({scrollTop:top},animateOptions);this.headerClicked=!1;self.startTabComponents(index)}}.bind(this));this.$sections.removeClass('active');this.sections[index].addClass('active')}else if(this.curLayout==='ver'){if(this.contents[this.active[0]]!==_undefined){this.contents[this.active[0]].css('display','none')}
this.contents[index].css('display','none').stop(!0,!0).fadeIn(this.options.duration,this._events.contentChanged);this.$sections.removeClass('active');this.sections[index].addClass('active');self.startTabComponents(index)}
this._events.contentChanged();this.$tabs.attr('aria-expanded','false').removeClass('active');this.tabs[index].attr('aria-expanded','true').addClass('active');this.active[0]=index;this.setBarPosition(index,!0)},toggleSection:function(index){var indexPos=$.inArray(index,this.active);if(indexPos!=-1){this.contents[index].css('display','block').slideUp(this.options.duration,this._events.contentChanged);this.tabs[index].attr('aria-expanded','true').removeClass('active');this.sections[index].removeClass('active');this.active.splice(indexPos,1)}else{this.contents[index].css('display','none').slideDown(this.options.duration,this._events.contentChanged);this.tabs[index].attr('aria-expanded','false').addClass('active');this.sections[index].addClass('active');this.active.push(index)}},startTabComponents:function(tabIndex){const self=this;$('.w-counter',self.sections[tabIndex]).each((_,item)=>{$(item).trigger('animation_start')})},resize:function(){this.width=this.isAccordionAtWidth?$us.$window.outerWidth():this.$container.width();if(this.curLayout!=='accordion'&&!this.width&&this.$container.closest('.w-nav').length&&!jQuery.isMobile){return}
...
const self=this;const $input=$(e.target);const isGroupTable=$input.closest('.cart').hasClass('grouped_form');const max=$ush.parseFloat($input.attr('max'))||-1;const min=$ush.parseFloat($input.attr('min'))||(isGroupTable?0:1);var value=$ush.parseFloat($input.val());if($input.is(':disabled')){return}
...
if(data.totals){const subtotalData={price:data.totals.total_items||'',precision:data.totals.currency_minor_unit??2,currency_code:data.totals.currency_code||'',currency_symbol:data.totals.currency_symbol||'',currency_prefix:data.totals.currency_prefix||'',currency_suffix:data.totals.currency_suffix||''};if($cartTotals.length){$('.cart-subtotal bdi',$cartTotals).html(self.formatPrice(subtotalData));const totalData={...subtotalData,price:data.totals.total_price};$('.order-total bdi',$cartTotals).html(self.formatPrice(totalData));const discountData={...subtotalData,price:data.totals.total_discount};$('.cart-discount .woocommerce-Price-amount',$cartTotals).html(self.formatPrice(discountData));const $taxRows=$('tr.tax-rate',$cartTotals);const taxLines=data.totals.tax_lines||[];if($taxRows.length&&taxLines.length===$taxRows.length){$taxRows.each((i,row)=>{const taxData={...subtotalData,price:taxLines[i].price};$('.woocommerce-Price-amount',row).first().html(self.formatPrice(taxData))})}else if($taxRows.length){const taxData={...subtotalData,price:data.totals.total_tax};$('.woocommerce-Price-amount',$taxRows.first()).html(self.formatPrice(taxData))}
...
const step=$ush.parseFloat($input.attr('step')||1);$input.val($ush.parseFloat($input.val())-step).trigger('change')},plusCartQuantity:function(e){const self=this;var $target=$(e.target),$input=$target.siblings('input.qty:first');if(!$input.length){return}
const step=$ush.parseFloat($input.attr('step')||1);$input.val($ush.parseFloat($input.val())+step).trigger('change')},updateCartForm_long:$ush.debounce($ush.fn,50),updateCartForm:function(){const self=this;if(typeof(lastUpdateCartRequest||{}).abort==='function'){lastUpdateCartRequest.abort()}
...
self._toggleCart(!1)},redirectToCartOnMobile:function(e){e.preventDefault();const cartUrl=String((window.wc_add_to_cart_params||{}).cart_url);if(cartUrl.includes(window.location.host)){window.location.replace(cartUrl)}},showCartOnClick:function(e){const self=this;e.preventDefault();if(['mouse','touch','pen'].includes(e.pointerType)||e.detail>0){self._toggleCart(!self.isCartOpen)}},showCartOnKeyup:function(e){const self=this;e.preventDefault();if(e.keyCode!==$ush.ENTER_KEYCODE){return}
...
return isVertical}});$.fn.us_product_gallery=function(){return this.each(function(){$(this).data('us_product_gallery',new usProductGallerySlider(this))})};$(()=>{const $wcSlider=$('.woocommerce-product-gallery.type_slider');if(!$wcSlider.length){return}
$ush.timeout(()=>$wcSlider.us_product_gallery(),200);$us.$body.on('wpcvi_show_selected_gallery wc_additional_variation_images_frontend_lightbox_done','form.variations_form',()=>{$ush.timeout(()=>{const $wcSliderVisible=$('.woocommerce-product-gallery:visible');if(!$wcSliderVisible.length||$wcSliderVisible.data('us_product_gallery')){return}
$wcSliderVisible.addClass($wcSlider.first().attr('class')).us_product_gallery()},0)})});$us.$document.on('usPopup.itemContentLoaded',(e,usPopup)=>{if(typeof $.fn.wc_product_gallery==='function'){const $wcProductGallery=$('.woocommerce-product-gallery',usPopup.$content);$wcProductGallery.wc_product_gallery();if($wcProductGallery.hasClass('type_slider')){$ush.timeout(()=>$wcProductGallery.us_product_gallery(),200)}}
