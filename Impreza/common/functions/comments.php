	...
	function us_comment_start( $comment, $args, $depth ) {
		...
		<li <?php comment_class( 'w-comments-item' ) ?> id="comment-<?php comment_ID() ?>">
			<div class="w-comments-item-meta">
				...
				<a class="w-comments-item-date smooth-scroll" href="#comment-<?php comment_ID() ?>"><?= get_comment_date() . ' ' . get_comment_time() ?></a>
			</div>
			<div class="w-comments-item-text">
				<?php if ( $comment->comment_approved == '0') { ?>
					<div class="w-message color_yellow"><?= us_translate( 'Your comment is awaiting moderation.' ) ?></div>
				<?php }
				comment_text() ?>
			</div>
