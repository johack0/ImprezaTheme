...
! function( $, undefined ) {
	...
	SE_Manager.prototype = {
		...
		getPosition: function( translateSpeed ) {
			...
			// Prevent element going beyond the bottom border of the document, because it causes scroll bar increasing glitch
		...
		setBlur: function() {
			...
			// Get blur direction
			var blur,
				blurDirection = self.getParam( 'blur_direction' ),
				blurSpeed = $ush.parseFloat( self.getParam( 'blur_speed' ) );

			if ( ! self.hasClass( 'has_blur' ) || ! blurDirection || ! blurSpeed ) {
				return;
			}
			...
			if ( $ush.parseInt( self.getParam( 'from_initial_position' ) ) == 1 ) {
				...
				// Get blur: RESULT = WINDOW SCROLL / ELEMENT INITIAL POSITION
				blur = min( 1, scroll().top / initialPosition );
				...
				// Get blur: RESULT = ( ( 100% / POSITION_DIFF ) * PASSED_POSITION ) / 100%
				blur = ( ( 100 / position.diff ) * ( currentPosition - startPosition ) ) / 100;
			}

			if ( blurDirection === 'out-in' ) {
				blur = 1 - blur; // reverse direction
			} else if ( blurDirection === 'out-in-out' ) {
				blur = ( 2 * blur ) - 1;
			} else if ( blurDirection === 'in-out-in' ) {
				blur = ( blur > 0.5 ? 2 : 0 ) - ( 2 * blur );
			}
			// 20px is base blur speed/strength that scales with settings multiplier, where 2px blur is minimal value
			self.style( '--blur', abs( ( blur * blurSpeed * 20 ) ).toFixed( 2 ) + 'px' );
		},
