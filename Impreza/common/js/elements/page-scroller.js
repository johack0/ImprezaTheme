...
;( function( $, _undefined ) {
	 ...
	 * @debounced
	 */
	const __debounce_fn_500ms = $ush.debounce( $ush.fn, /* wait */500 );
	...
	$us.PageScroller.prototype = {
		init: function( container, options ) {
			...
			$us.$window
				.on( 'scroll.noPreventDefault', $ush.debounce( this._events.scroll, 5 ) )
				.on( 'resize', ( _, stopExecute ) => {
					if ( ! stopExecute ) {
						_resize();
					}
				} );
		...
		_mouseWheelHandler: function( e ) {
			...
			self.scrolls.push( abs( direction ) );
		...
		_attachEvents: function() {
			...
			if (
				...
				if ( $.isMobile || self.isTouch ) {
					...
					self.touchStartPageY = 0;
					self.touchClientX = 0;
					self.touchClientY = 0;

					$us.$canvas.on( 'touchstart.noPreventDefault', ( e ) => {
						const originalEvent = e.originalEvent;
						if ( $ush.isUndefined( originalEvent.pointerType ) || originalEvent.pointerType !== 'mouse' ) {
							self.touchStartPageY = originalEvent.touches[0].pageY;
							self.touchClientX = originalEvent.touches[0].clientX;
							self.touchClientY = originalEvent.touches[0].clientY;
						}
					} );

					$us.$canvas.on( 'touchmove', function( e ) {

						e.preventDefault();

						const time = $ush.time();
						const originalEvent = e.originalEvent;
						const touchEndPageY = originalEvent.touches[0].pageY;

						var currentX = originalEvent.touches[0].clientX,
							currentY = originalEvent.touches[0].clientY,
							diffX = self.touchClientX - currentX,
							diffY = self.touchClientY - currentY;
						// Swipe left or right to exit.
						if ( abs( diffX ) > abs( diffY ) ) {
							return;
						}
						self.touchClientX = currentX;
						self.touchClientY = currentY;

						var target = self.activeSection;

						if ( abs( self.touchStartPageY - touchEndPageY ) > ( $us.$window.height() / 50 ) ) {
							if ( self.touchStartPageY > touchEndPageY ) {
								target ++;
							} else if ( touchEndPageY > self.touchStartPageY ) {
								target --;
							}
							if ( $ush.isUndefined( self.sections[ target ] ) ) {
								return;
							}
							self.scrollTo( target );
		...
		scrollTo: function( target ) {
			...
			// For dots points from sticky block.
	...
	$( function() {
		...
		$ush.timeout( () => $( '.w-scroller' ).usPageScroller(), 0 );
	} );
