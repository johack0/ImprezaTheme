...
! function( $ ) {
	...
	$us.WLogin.prototype = {
		init: function( container, options ) {
			...
			this.$showPasswordBtn = this.$container.find( '.for_password .show-password-btn' );
			...
			this._events = {
				...
				toggleViewPassword: this.toggleViewPassword.bind( this ),
			};

			this.$container.on( 'submit', this._events.formSubmit );
			this.$showPasswordBtn.on( 'click', this._events.toggleViewPassword );
		...
		toggleViewPassword: function( event ) {
			event.preventDefault();
			if ( this.$showPasswordBtn.hasClass( 'display-password' ) ) {
				this.$password.prop( "type", "password" );
				this.$showPasswordBtn.attr( 'aria-label', this.$showPasswordBtn.data( 'show-password-translated' ) );
			} else {
				this.$password.prop( "type", "text" );
				this.$showPasswordBtn.attr( 'aria-label', this.$showPasswordBtn.data( 'hide-password-translated' ) );
			}
			this.$showPasswordBtn.toggleClass( 'display-password' );
			this.$password.focus();
		}
	};
