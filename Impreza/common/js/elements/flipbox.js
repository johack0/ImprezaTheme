...
! function( $ ) {
	...
	$us.WFlipBox = function( container ) {
		...
		// For diagonal cube animations height should equal width (geometrical restriction)
	...
	$us.WFlipBox.prototype = {
		...
		makeHoverable: function( exclude ) {
			...
			if ( jQuery.isMobile ) {
				...
				this.$container.on( 'touchstart.noPreventDefault', this._events.touchHoverStart );
				if ( exclude ) {
					this._events.touchHoverPrevent = function( e ) {
						e.stopPropagation();
					};
					this.$container.find( exclude ).on( 'touchstart.noPreventDefault', this._events.touchHoverPrevent );
				}
			} else {
