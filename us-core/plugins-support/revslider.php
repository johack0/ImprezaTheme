...
if (
	...
	OR ( defined( 'RS_REVISION' ) AND preg_match( '/^7\.\d+\.\d+$/', RS_REVISION ) )
) {
	return;
}
	...
	function us_include_revslider_js_for_row_bg() {
		...
		if ( $isPutIn === FALSE ) {
			...
			if (
				...
				AND is_numeric( $page_template_id = us_get_page_area_id( 'content' ) )
				AND get_post_status( $page_template_id ) == 'publish'
			) {
				$is_slider_on_page_block = us_find_element_in_post_page_blocks( $page_template_id, '[rev_slider' );
			}
