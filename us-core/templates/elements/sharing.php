 ...
 * @param $twitter        bool Is X (Twitter) button available?
...
foreach ( $providers_list as $provider => $provider_data ) {
	...
	$api_url = str_replace( '{{text}}', urlencode( get_the_title() ), $api_url );
	if ( $post_thumbnail ) {
	...
	$item_atts = array(
		...
		'href' => $api_url,
		'title' => apply_filters( 'us_sharing_item_title', $provider_data['title'], $provider ),
		'aria-label' => apply_filters( 'us_sharing_item_title', $provider_data['title'], $provider ),
	);
