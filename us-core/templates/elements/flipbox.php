...
// Link
$anchor_html = $btn_html = '';
$link_atts = us_generate_link_atts( $link );

if ( ! empty( $link_atts['href'] ) OR usb_is_post_preview() ) {
	if ( $link_type == 'container' ) {
		$anchor_html = '<a' . us_implode_atts( $link_atts ) . '></a>';

	} elseif ( $link_type == 'btn' AND $btn_label = us_replace_dynamic_value( $btn_label ) ) {
		$btn_params = array(
			'html_atts' => array(
		...
		$btn_html .= us_get_btn( $btn_params );
	}
}
...
$output .= '</div>'; // .w-flipbox-hh
$output .= '</div>'; // .w-flipbox-h
$output .= $anchor_html;
$output .= '</div>'; // .w-flipbox
