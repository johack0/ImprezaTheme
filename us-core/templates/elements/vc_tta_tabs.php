...
$func_parse_vc_tta_section = function( $matches ) use( &$us_tabs_atts, &$active_tab_indexes, &$section_contents ) {
	...
	if ( isset( $item_atts['conditions_operator'] ) AND $item_atts['conditions_operator'] == 'never' ) {
		$item_atts['conditions'] = array(); // prevent unset array key access
	}
...
if ( $shortcode_base != 'vc_tta_accordion' ) {
	...
	$_atts['class'] .= ' toggle_style_1'; // default toggle style
}
...
if ( $shortcode_base == 'vc_tta_accordion' ) {
	...
	// Pass Button Style to sections
	if ( is_numeric( $toggle_style ) ) {

		$_atts['class'] .= ' toggle_style_btn';

		$section_header_btn_class = ' w-btn ' . us_get_btn_class( $toggle_style );

		// Disable border gradient animation (cause it is not working here)
		$section_header_btn_class = str_replace( ' with_border_animation', '', $section_header_btn_class );

		$us_tabs_options[ $current_tabs_index ]['section_header_btn_class'] = $section_header_btn_class;

	} else {
		$_atts['class'] .= ' toggle_' . $toggle_style;
	}

	// Gap between sections
	$_atts['style'] .= '--sections-gap:' . $sections_gap . ';';
