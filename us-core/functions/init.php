...
function uscore_after_setup_theme() {
	...
	// Helper functions for 'loop' elements and Grid Layouts
	require US_CORE_DIR . 'functions/list.php';

	// Methods that apply to Grid and Grid Filter (DEPRECATED)
	...
	// Filter cache
	require US_CORE_DIR . 'functions/filter-cache.php';
	...
	// MCP server (AI Assistant). Self-disables if WP < 6.9 or Composer deps missing.
	require US_CORE_DIR . 'mcp/bootstrap.php';
	...
	if ( wp_doing_ajax() ) {
		...
		require US_CORE_DIR . 'functions/ajax/popup.php';
	}
