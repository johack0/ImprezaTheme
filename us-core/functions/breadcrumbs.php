...
class US_Breadcrumbs {
	...
	public function render() {
		...
		// Always show dummy breadcrumbs in Live Preview for better UX
		if ( usb_is_preview() ) {
			if ( ! empty( $this->config['home'] ) ) {
				$output .= $this->get_breadcrumb( '#', $this->config['home'] ) . $this->config['delimiter'];
			}
			$output .= $this->get_breadcrumb( '#', us_translate( 'Parent Category' ) );
			$output .= $this->config['delimiter'] . $this->get_breadcrumb( '#', us_translate( 'Category' ) );
			$output .= $this->config['delimiter'] . $this->get_breadcrumb( '#', get_the_title() );
		...
		} elseif ( $post instanceof WP_Post_Type ) {
			$post_type = $post->name;
		} else {
			$post_type = 'page';
		}
		...
		if ( is_home() ) {
			...
			$output .= $this->get_breadcrumb( get_permalink( $post->ID ?? 0 ), get_the_title( $post->ID ?? 0 ) );
			...
			$output .= $this->config['before_basic'] . $post->name . $this->config['after_basic'];
