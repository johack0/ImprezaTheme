...
function us_core_updade_action( $upgrader_object, $options ) {
	if ( $options['action'] == 'update' && $options['type'] == 'plugin' AND ! empty( $options['plugins'] ) AND is_array( $options['plugins'] ) ) {
		foreach ( $options['plugins'] as $plugin_name ) {
			if ( $plugin_name == 'us-core/us-core.php' ) {
...
if ( ! function_exists( 'us_activate_maintenance_mode_for_dev_license' ) ) {
	add_action( 'init', 'us_activate_maintenance_mode_for_dev_license' );
	/**
	 * Always activate the maintenance mode for developer license
	 */
	function us_activate_maintenance_mode_for_dev_license() {
		if ( get_option( 'us_license_dev_activated', 0 ) AND ! us_get_option( 'maintenance_mode' ) ) {
