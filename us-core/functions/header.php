...
if ( ! function_exists( 'us_header_settings_fallback' ) ) {
	...
	function us_header_settings_fallback( $header_settings ) {
		...
		// Fallback for width (after version 8.45.1)
		if ( ! empty( $header_settings['default']['options']['top_fullwidth'] ) ) {
			$header_settings['default']['options']['top_width'] = 'full_with_indents';
			unset( $header_settings['default']['options']['top_fullwidth'] );
		}
		if ( ! empty( $header_settings['default']['options']['middle_fullwidth'] ) ) {
			$header_settings['default']['options']['middle_width'] = 'full_with_indents';
			unset( $header_settings['default']['options']['middle_fullwidth'] );
		}
		if ( ! empty( $header_settings['default']['options']['bottom_fullwidth'] ) ) {
			$header_settings['default']['options']['bottom_width'] = 'full_with_indents';
			unset( $header_settings['default']['options']['bottom_fullwidth'] );
		}
	...
	function us_fix_header_settings( $value ) {
		...
		foreach ( us_config( 'header-settings.options', array() ) as $group => $opts ) {
			foreach ( $opts as $opt_name => $opt ) {
				$options_defaults[ $opt_name ] = $opt['std'] ?? '';
			}
		}
...
if ( ! function_exists( 'us_load_header_settings' ) ) {
	...
	function us_load_header_settings( $header_settings ) {
		...
		if (
			...
			$func_apply_header_transparent_options( $page_template_id );
		}
		...
		if ( ! empty( $postID ) AND $postID != 'default' ) {
			if (
				...
				// Remove content offset options from a post metadata
				if ( get_post_meta( $postID, 'us_remove_header_offset_override', TRUE ) ) {
					$states_without_offset = get_post_meta( $postID, 'us_remove_header_offset', TRUE );

					// The value can be either an array or a delimited string
					if ( is_string( $states_without_offset ) ) {
						$states_without_offset = explode( ',' , $states_without_offset );
					}
					foreach ( $states as $state ) {
						$override_options[ $state ]['options']['remove_header_offset'] = in_array( $state, (array) $states_without_offset );
					}
				}
...
if ( ! function_exists( 'us_output_builder_elms' ) ) {
	 ...
	 * @param string $loop_item_type 'post' / 'term' / 'user'
	 */
	function us_output_builder_elms( &$settings, $state, $place, $context = 'header', $grid_has_overriding_link = FALSE ) {
		...
		foreach ( $layout[ $place ] as $elm ) {
			...
			// Dynamic values support in element ID field
			if ( ! empty( $data[ $elm ]['el_id'] ) ) {
				$data[ $elm ]['el_id'] = us_replace_dynamic_value( $data[ $elm ]['el_id'] );
			}
			...
			if ( ! empty( $data[ $elm ]['scroll_effect'] ) ) {
				...
				// Blur
				if ( ! empty( $data[ $elm ]['scroll_blur'] ) ) {
					$classes .= ' has_blur';
					$us_scroll_params['blur_direction'] = $data[ $elm ]['scroll_blur_direction'];
					$us_scroll_params['blur_speed'] = $data[ $elm ]['scroll_blur_speed'];
				}
			...
			if (
				! empty( $data[ $elm ]['enable_custom_html_atts'] )
				AND ! empty( $data[ $elm ]['custom_html_atts'] )
				AND is_array( $data[ $elm ]['custom_html_atts'] )
			) {
				$html_atts = array();
				foreach ( $data[ $elm ]['custom_html_atts'] as $attr ) {
					if ( ! empty( $attr['name'] ) ) {
						...
						$html_atts[ $attr['name'] ] = us_replace_dynamic_value( $attr['value'] ?? '' );
					}
				}
				if ( ! empty( $html_atts ) ) {
					$data[ $elm ]['_atts'] = $html_atts;
				}
			}
			...
			// Reset link for inner elements if overriding link is set
			if ( $context === 'grid' AND $grid_has_overriding_link ) {
				$data[ $elm ]['link'] = '';
				
				// Buttons are focusable even without a link, disabling the focus
				if ( $elm_name === 'btn' ) {
					$data[ $elm ]['_atts'] = $data[ $elm ]['_atts'] ?? array();
					$data[ $elm ]['_atts']['tabindex'] = -1;
				}
			}
			...
			if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
				...
				if ( isset( $data[ $elm ] ) ) {
					...
					if ( isset( $data[ $elm ]['inner_items_gap'] ) ) {
						...
						if ( strpos( $elm, 'hwrapper' ) !== FALSE ) {
							...
							// Save the gap value for child wrappers so they have correct bottom offset
							static $parent_wrapper_gap;
							if ( $parent_wrapper_gap ) {
								$wrapper_atts['style'] .= " --vwrapper-parent-gap: $parent_wrapper_gap;";
							}
							$parent_wrapper_gap = $inner_items_gap;
						}
					}
					if ( ! empty( $data[ $elm ]['el_id'] ) ) {
						$wrapper_atts['id'] = $data[ $elm ]['el_id'];
					}
				...
				us_output_builder_elms( $settings, $state, $elm, $context, $grid_has_overriding_link );
				echo $link_html ?? '';
				echo '</div>';

				// Reset static variable to not apply on the first parent wrapper
				$parent_wrapper_gap = NULL;
				...
				$values['id'] = $elm;
				$values['atts'] = $values; // required in the shortcode template
				$values['classes'] = ( $values['classes'] ?? '' ) . $classes;
	...
	function us_get_elm_defaults( $type, $context = 'header' ) {
		...
		if ( ! isset( $us_elm_defaults[ $context ][ $type ] ) ) {
			...
			foreach ( us_arr_path( $elm_config, 'params', array() ) as $field_name => $field ) {
				...
				$value = $field[ $context . '_std' ] ?? $value;
...
if ( ! function_exists( 'us_get_header_design_options_css' ) ) {
	...
	function us_get_header_design_options_css() {
		...
		return us_compile_css( $jsoncss_collection, $breakpoints );
	}
}
...
if ( ! function_exists( 'us_admin_bar_menu' ) ) {
	...
	function us_admin_bar_menu( WP_Admin_Bar $wp_admin_bar ) {
		...
		if (
			...
			us_get_recursive_page_blocks( $current_page, $func_acc_page_blocks );
		}
		unset( $current_page );
		...
		foreach ( array_keys( $area_names ) as $area ) {
			if ( $area == 'page_blocks' AND $page_block_ids = (array) us_get_current_page_block_ids() ) {
				...
				foreach ( get_posts( $query_args ) as $post ) {
					// Recursive get of all Reusable Blocks
					us_get_recursive_page_blocks( $post, $func_acc_page_blocks );
				}
