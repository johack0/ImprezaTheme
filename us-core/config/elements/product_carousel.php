...
/**
 * Configuration for shortcode: us_product_carousel
 */

// Get params from Product List and Carousel options to avoid params duplication
$product_list_params = $conditional_params = $design_options_params = $responsive_params = array();

if ( us_is_elm_editing_page() ) {
	$product_list_params = us_config( 'elements/product_list.params' );
...
foreach( $product_list_params as $_param_name => &$_param ) {

	if ( in_array( $_param_name, array_keys( $conditional_params ) ) ) {
		unset( $product_list_params[ $_param_name ] );
	}
