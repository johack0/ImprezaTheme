	...
	function us_menu_item_custom_fields( $id, $item, $depth, $args ) {
		...
		if ( $item->object == 'us_page_block' ) { ?>
			<p class="field-us-custom">
				<label for="edit-menu-item-remove-rows-<?= $id ?>">
					<input type="checkbox" id="edit-menu-item-remove-rows-<?= $id ?>" name="menu-item-remove-rows[<?= $id ?>]"<?php checked( get_post_meta( $id, '_menu_item_remove_rows', TRUE ) ) ?> />
					<?= strip_tags( __( 'Exclude Rows and Columns', 'us' ) ) ?>
				</label>
			</p>
		<?php }
