...
// Add Google Fonts css file for Gutenberg
add_action( 'enqueue_block_assets', 'us_enqueue_google_fonts' );

// Add custom css file for Gutenberg
add_action( 'enqueue_block_assets', 'us_include_generated_styles_for_gutenberg' );
function us_include_generated_styles_for_gutenberg() {
	if ( ! is_admin() AND ! us_get_option( 'block_editor' ) ) {
		return;
	}
