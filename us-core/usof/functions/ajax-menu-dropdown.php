...
function usof_ajax_mega_menu_save_settings() {
	...
	$settings = array_map( 'strip_tags', $_POST['settings'] );
...
<?php
	...
	$values = get_post_meta( $item_id, 'us_mega_menu_settings', TRUE ) ?: array();
		...
		const USMMSave = () => {
			...
			$.ajax({
				...
				data: {
					...
					security: '<?= wp_create_nonce( 'us-menu-dropdown-save' ) ?>'
				},
