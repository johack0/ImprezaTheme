...
self._setScreenWidth(width);self.$inputHeight.val(ceil($usb.$iframe.height()))},setFieldResponsiveScreen:$ush.debounce(function(){const self=this;if(self.isVisible()){self.trigger('document.syncResponsiveState',_$tmp.currentResponsiveScreen)}},100),fieldSetResponsiveScreen:function(screen){const self=this;if(!self.isVisible()){self.showToolbar(screen)}else{self.setResponsiveScreen(screen)}
$usb.urlManager.setParam('screen',screen).push()},_maybeStartResize:function(e){const self=this;if(!self.isVisible()){return}
const $screen=self.$screen;$usbcore.cache('temp').set('resizeData',{resizeControl:$usbcore.$attr(e.target,'data-resize-control'),startHeight:$ush.parseInt($screen.height()),startWidth:$ush.parseInt($screen.width()),startPageX:e.pageX,startPageY:e.pageY});$screen.addClass('resizable')},_maybeResize:function(e){const self=this;if(!self.isVisible()){return}
...
self.$inputHeight.val(height-resizeControlWidth);self.$screen.css('height',height)}},_endResize:function(){const self=this;if(!self.isVisible()){return}
...
const href=self._getGoogleEndpoint(themeOptions);const $node=$('link[id='+$usb.config('typography.fonts_id')+']',$usb.iframe.contentDocument);if($node.length){$node.attr('href',href)}else{$('head',$usb.iframe.contentDocument).append('<link id="'+$usb.config('typography.fonts_id')+'" rel="stylesheet" href="'+href+'" media="all">')}},_getGoogleEndpoint:function(themeOptions){const self=this;var config=$usb.config('typography',{}),axesMap=config.googleFontsAxes||{},usedFonts=self._getUsedFonts(themeOptions),families=[];for(const fontFamily in usedFonts){families.push(self._buildFamily(fontFamily,usedFonts[fontFamily],axesMap[fontFamily]))}
if(families.length){return config.googleapis_v2+'?'+families.join('&')+'&display='+config.font_display}
return''},_getUsedFonts:function(themeOptions){var config=$usb.config('typography',{}),googleFonts=config.googleFonts||{},additionalGoogleFonts=config.usedGoogleFonts||{},usedFonts={};var tags=config.tags||[];for(const i in tags){var tagProps=themeOptions[tags[i]];if(!$.isPlainObject(tagProps)){continue}
var fontFamily=tagProps['font-family'];if($ush.isUndefined(fontFamily)||fontFamily==='inherit'){continue}
if($ush.isUndefined(googleFonts[fontFamily])){continue}
if($ush.isUndefined(usedFonts[fontFamily])){usedFonts[fontFamily]=$ush.toString(googleFonts[fontFamily]).split(',')}}
for(const googleFont in additionalGoogleFonts){if($ush.isUndefined(usedFonts[googleFont])){usedFonts[googleFont]=$ush.toString(additionalGoogleFonts[googleFont]).split(',')}}
return usedFonts},_buildFamily:function(fontFamily,variants,axes){variants=variants||[];var encodedFamily=fontFamily.split(' ').join('+');var hasItalic=(''+variants.join(',')).indexOf('italic')>-1;var tagList,tuples;if($.isPlainObject(axes)){var tags=Object.keys(axes).sort(function(a,b){var aLower=a.charAt(0)>='a'&&a.charAt(0)<='z';var bLower=b.charAt(0)>='a'&&b.charAt(0)<='z';if(aLower!==bLower){return aLower?-1:1}
return a<b?-1:(a>b?1:0)});var rangeRow=tags.map(function(tag){return axes[tag].min+'..'+axes[tag].max});tagList=hasItalic?['ital'].concat(tags):tags;tuples=hasItalic?['0,'+rangeRow.join(','),'1,'+rangeRow.join(',')]:[rangeRow.join(',')]}else if(hasItalic){tagList=['ital','wght'];var pairs={};variants.forEach(function(variant){var ital=(''+variant).indexOf('italic')>-1?1:0;var weight=$ush.parseInt(variant)||400;pairs[ital+','+weight]=[ital,weight]});tuples=Object.keys(pairs).map(function(key){return pairs[key]}).sort(function(a,b){return a[0]-b[0]||a[1]-b[1]}).map(function(pair){return pair[0]+','+pair[1]})}else{tagList=['wght'];var seen={},weights=[];variants.forEach(function(variant){var weight=$ush.parseInt(variant)||400;if(!seen[weight]){seen[weight]=!0;weights.push(weight)}});weights.sort(function(a,b){return a-b});if(!weights.length){weights=[400]}
tuples=weights.map(String)}
return'family='+encodedFamily+':'+tagList.join(',')+'@'+tuples.join(';')}});$usb.fonts=new Fonts}(jQuery);!function($,_undefined){const _window=window;const _document=document;const abs=Math.abs;const ceil=Math.ceil;_window.$ush=_window.$ush||{};_window.usGlobalData=_window.usGlobalData||{};DIRECTION_TOP='top';DIRECTION_BOTTOM='bottom';const REGEXP_USBID_ALIAS=/^([\w\-]+:\d+)\|([a-z\d\-]+)$/;const REGEXP_SPACE=/\p{Zs}/u;const REGEXP_USBID=/(\s?usbid="([^\"]+)?")/g;var _$mode='preview';const _defaultPageData={content:'',customCss:'',fields:{},postMeta:{}};const _$tmp={usedElementIds:[],isInitDragDrop:!1,isProcessSave:!1,savedPageData:$ush.clone(_defaultPageData),transit:null,customTransit:null,movedElements:{},};const elementsMap=new Map;function Builder(){const self=this;self.defaultRootContainer='root_container';self.postContentRootContainer='post_content_root_container';self.rootContainer=self.defaultRootContainer;self.isRemoveRows=!1;self.selectedElmId;self._isReloadPreviewAfterSave=!1;self.pageData=$ush.clone(_defaultPageData);_window.onbeforeunload=(e)=>{if(self.isPageChanged()){e.preventDefault();return $usb.getTextTranslation('page_leave_warning')}};self._events={dragstart:self._dragstart.bind(self),iframeReady:self._iframeReady.bind(self),maybeDrag:self._maybeDrag.bind(self),maybeStartDrag:self._maybeStartDrag.bind(self),modeChanged:self._modeChanged.bind(self),historyChanged:self.historyChanged.bind(self),dropzoneAddRow:self.dropzoneAddRow.bind(self),elmCopy:self._elmCopy.bind(self),elmDelete:self._elmDelete.bind(self),elmDuplicate:self._elmDuplicate.bind(self),elmPaste:self._elmPaste.bind(self),elmSelected:self.elmSelected.bind(self),endDrag:self._endDrag.bind(self),};$(()=>{self.$container=$('#usb-wrapper');$usb.$document.on('dragstart',self._events.dragstart);_$tmp.customTransit=document.querySelector('.usb-custom-transit')});$usb.on('iframeReady',self._events.iframeReady).on('builder.modeChanged',self._events.modeChanged).on('builder.endDrag',self._events.endDrag).on('builder.elmCopy',self._events.elmCopy).on('builder.elmPaste',self._events.elmPaste).on('builder.elmDelete',self._events.elmDelete).on('builder.elmDuplicate',self._events.elmDuplicate).on('builder.elmSelected',self._events.elmSelected).on('builder.dropzoneAddRow',self._events.dropzoneAddRow).on('history.changed',self._events.historyChanged)};const prototype=Builder.prototype;$.extend(prototype,$ush.mixinEvents,{_modeChanged:function(){$usb.postMessage('doAction','hideHighlight')},_iframeReady:function(){const self=this;if(!$usb.iframeIsReady){return}
const iframeWindow=$usb.iframe.contentWindow;if((iframeWindow.location.search||'').indexOf('&meta')!==-1){return}
$usb.postMessage('doAction','hideHighlight');self.pageData=$ush.clone((iframeWindow.usGlobalData||{}).pageData||{},_defaultPageData);_$tmp.savedPageData=$ush.clone(self.pageData);$ush.timeout(self.reloadElementsMap.bind(self),0)},elmSelected:function(id){const self=this;if(!self.isMode('editor')||!self.doesElmExist(id)){return}
...
const content=$ush.toString(self.getElmShortcode(id));$ush.copyTextToClipboard(content.replace(/\susbid="([^\"]+)"/gi,''));$ush.storage('usb').set('сlipboard',content)},_elmPaste:function(id){const self=this;if(!self.isValidId(id)&&!self.isRootContainer(id)){return}
...
if(self.isRemoveRows){content=$usb.builder.removeRows(content)}
...
self.removeElm(id)},historyChanged:function(task){const self=this;if($.isPlainObject(task)&&task.type===HISTORY_TYPE.REDO){elementsMap.set(task.id,{parentId:task.parentId,elmIndex:task.index})}},dropzoneAddRow:function(){const self=this;self.createElm('vc_column_text',self.rootContainer,0,{},self.elmSelected)},});$.extend(prototype,{_dragStartDistance:5,_dragstart:function(e){return $(e.target).closest('.media-frame').length>0},initDragDrop:function(){const self=this;if(_$tmp.isInitDragDrop){return}
...
if($usb.panel.isVisible()&&$ush.isFirefox&&$usb.preview.getCurrentOffset().x>=e.clientX){self._clearDragAssets();return}
if(!self.isParentDragging()){$usbcore.cache('drag').flush();return}
if(!!dragData.parentId&&!!dragData.currentId){var maybeIndex=$ush.parseInt(dragData.maybeIndex);if(self.isAliasElmId(dragData.parentId)){dragData.parentId=self.removeAliasFromId(dragData.parentId)}
var templateId=$usbcore.$attr(dragData.target,'data-template-id'),favoriteSectionId=$usbcore.$attr(dragData.target,'data-section-id');if(templateId){var templateCategoryId=$(dragData.target).closest('.usb-template').data('template-category-id');$usb.templates.insertTemplate(templateCategoryId,templateId,dragData.parentId,maybeIndex)}else if(favoriteSectionId){$usb.favorites.insertSection(favoriteSectionId,dragData.parentId,maybeIndex)}else{self.createElm(self.getElmType(dragData.currentId),dragData.parentId,maybeIndex)}
...
parseShortcode(content,self.rootContainer)},hasElementId:function(id){return id&&elementsMap.get(id)!==_undefined},isProcessSave:function(){return _$tmp.isProcessSave},savePageData:function(complete){const self=this;var data={postMeta:{},};if(self.isContentChanged()){data.post_content=self.pageData.content}
...
if(self.isRemoveRows&&self.isPostContentRootContainer(self.rootContainer)){data.remove_rows=1}
...
return result.join(' ')},isValidId:function(id){return id&&/^([\w\-]+):(\d+)(\|[a-z\-]+)?$/.test(id)},isRow:function(id){return this.getElmName(id)==='vc_row'},isRowInner:function(id){return this.getElmName(id)==='vc_row_inner'},isColumn:function(id){return/^vc_column(_inner)?$/.test(this.getElmName(id))},isContentCarousel:function(id){return this.getElmName(id)==='content_carousel'},isContentCarouselSingleSlide:function(id){return this.isContentCarousel(id)&&this.getElmShortcode(id).includes('items="1"')},isOutsideRootContainer:function(id){return $usb.config('elms_outside_root_container',[]).includes(this.getElmName(id))},isDefaultRootContainer:function(id){return id&&id===this.defaultRootContainer},isPostContentRootContainer:function(id){return id&&id===this.postContentRootContainer},isRootContainer:function(id){return this.isDefaultRootContainer(id)||this.isPostContentRootContainer(id)},setRootContainer:function(rootContainer){const self=this;if(![self.defaultRootContainer,self.postContentRootContainer].includes(rootContainer)){rootContainer=self.defaultRootContainer}
if(self.rootContainer!==rootContainer){self.rootContainer=rootContainer;self.reloadElementsMap()}},isElmContainer:function(name){const self=this;if(self.isValidId(name)){name=self.getElmName(name)}
...
return name==='vc_tta_section'},isElmWrapper:function(name){const self=this;if(self.isValidId(name)){name=self.getElmType(name)}
return/^us_(h|v)wrapper$/.test(name)},_escapeAttr:function(value){return $ush.toString(value).replace(/\[/g,'&#91;').replace(/\]/g,'&#93;')},_unescapeAttr:function(value){return $ush.toString(value).replace(/&#91;/g,'[').replace(/&#93;/g,']')},canBeChildOf:function(id,parent,strict){const self=this;const isRootContainer=self.isRootContainer(parent);if(self.isRootContainer(id)||!self.isValidId(id)||!(self.isValidId(parent)||isRootContainer)){return!1}
if(self.isMode('drag:move')&&self.isPostContentRootContainer(parent)&&!self.isRow(id)){return!1}
var result=!0;const name=self.getElmName(id);const parentName=(isRootContainer?parent:self.getElmName(parent));const shortcodeRelations=$.extend({},$usb.config('shortcode.relations',{}));if($.isEmptyObject(shortcodeRelations)){return!0}
return self._cacheDragProcessData(()=>{for(var k in shortcodeRelations){if(!result){break}
var relations=shortcodeRelations[k][k==='as_child'?name:parentName];if($ush.isUndefined(relations)){continue}
for(const condition in relations){if(self.isMode('drag:add')&&self.isRootContainer(parentName)&&!self.isChildElmContainer(id)){continue}
if(!result){break}
const allowed=(relations[condition]||'').split(',');const isFound=allowed.includes(k==='as_child'?parentName:name);if((condition==='only'&&!isFound)||(condition==='except'&&isFound)){result=!1}}}
if(result&&!!strict&&(isRootContainer||self.isChildElmContainer(id))){const hasMovedElements=(self.isMode('drag:move')&&self.hasMovedElements());var movedElementsBackup;if(hasMovedElements){movedElementsBackup=$ush.clone(_$tmp.movedElements);self.restoreMovedElements()}
...
return result},`canBeChildOf:${id}|${parent}|${strict}`,!1)},hasSameTypeParent:function(type,parent){const self=this;if(self.isRootContainer(type)||self.isRootContainer(parent)||!self.isValidId(parent)){return!1}
if(self.isValidId(type)){type=self.getElmType(type)}
if(type===self.getElmType(parent)){return!0}
var iteration=0;while(parent!==null||self.isRootContainer(parent)){if(iteration++>=1000){break}
parent=self.getElmParentId(parent);if(self.getElmType(parent)===type){return!0}}
return!1},getValidContainerId:function(container){return this.isElmContainer(container)?container:this.rootContainer},isAliasElmId:function(id){const self=this;if(!self.isValidId(id)){return!1}
return REGEXP_USBID_ALIAS.test(id)},getAliasFromId:function(id){const self=this;if(!self.isValidId(id)){return null}
return(id.match(REGEXP_USBID_ALIAS)||[])[2]||null},addAliasToElmId:function(alias,id){const self=this;const args=arguments;if(alias&&typeof alias==='string'&&self.isValidId(id)){id+='|'+alias}else{$usb.log('Notice: Failed to add alias to id',args)}
return id},removeAliasFromId:function(id){const self=this;if(!self.isValidId(id)){return id}
return(id.match(REGEXP_USBID_ALIAS)||[])[1]||id},getElmType:function(id){const self=this;if($ush.isNode(id)){id=self.getElmId(id)}
...
if(!node.hasOwnProperty('_$$usbid')){const id=$usbcore.$attr(node,'data-usbid');node._$$usbid=(self.isValidId(id)||self.isRootContainer(id))?id:''}
...
for(var index=1;;index++){const newElementId=type+':'+index;if(!self.doesElmExist(newElementId)&&!_$tmp.usedElementIds.includes(newElementId)){_$tmp.usedElementIds.push(newElementId);return newElementId}}},getElmParentId:function(id){const self=this;if(!self.doesElmExist(id)||id===self.rootContainer){return null}
if(elementsMap.has(id)){return elementsMap.get(id).parentId||null}
return null},getElmSiblingsId:function(id){const self=this;if(!self.isValidId(id)||self.isRootContainer(id)){return[]}
...
shortcodeObj.atts=' '+self.buildAtts($.extend(self.getElmValues(id),values));self.pageData.content=$ush.toString(self.pageData.content).replace(shortcode,self.buildShortcode(shortcodeObj));$usb.trigger('builder.contentChange')},removeRows:function(content){content=String(content);if(content.includes('vc_row_inner')){return content.replace(/\[\/?vc_(?:row|column)\b[^\]]*\]/g,'')}
return content.replace(/(\[\/?vc_(?:row|column)\b)([^\]]*\])/g,'$1_inner$2').replace(/"(vc_(?:row|column)\b)(:[\d]+)"/g,'"$1_inner$2"')},_cacheDragProcessData:function(callback,key,defaultValue){const self=this;if(typeof callback!=='function'){return defaultValue}
...
var columns=self.getElmChildren(rowId),columnsCount=columns.length,renderNeeded=!1,columnType=self.isRow(rowId)?'vc_column':'vc_column_inner',newColumnsWidths=[],newColumnsWidthsBase=0,newColumnsWidthsTmp,newColumnsCount;layout=$ush.toString(layout);if(layout.includes('-')){newColumnsWidthsTmp=layout.split('-');newColumnsCount=newColumnsWidthsTmp.length;for(var i=0;i<newColumnsCount;i ++){newColumnsWidthsBase+=$ush.parseInt(newColumnsWidthsTmp[i])}
for(var i=0;i<newColumnsCount;i ++){var columnWidthBaseTmp=newColumnsWidthsBase/newColumnsWidthsTmp[i];if(columnWidthBaseTmp%1===0){newColumnsWidths.push('1/'+columnWidthBaseTmp)}else{newColumnsWidths.push(newColumnsWidthsTmp[i]+'/'+newColumnsWidthsBase)}}}else if(!layout.includes('(')&&REGEXP_SPACE.test(layout)){const customColumns=layout.trim().split(REGEXP_SPACE).filter((value)=>/^(?:\d+(?:\.\d+)?(?:px|rem|em|%)|\d+fr|auto)$/.test(value));newColumnsCount=0;for(var i in customColumns){newColumnsWidths.push('1/1');newColumnsCount++}}else{if($ush.isNumeric(layout)){newColumnsCount=$ush.parseInt(layout)}else{newColumnsCount=layout.split(REGEXP_SPACE).length}
if(newColumnsCount>10){newColumnsCount=10}
...
return renderNeeded},getInsertPosition:function(parent,index){const self=this;const isParentContainer=self.isElmContainer(parent);var position;index=$ush.parseInt(index);if(self.isRootContainer(parent)||isParentContainer){const children=self.getElmChildren(parent);if(index===0||children.length===0){position='prepend'}else if(index>children.length||children.length===1){index=children.length;position='append'}else{parent=children[index-1]||parent;position='after'}}else{position=(index<1?'before':'after')}
return{position:position,parent:parent}},insertShortcodeIntoContent:function(parent,index,newShortcode){const self=this;if(!newShortcode||!(self.isValidId(parent)||self.isRootContainer(parent))){return!1}
const insertPosition=self.getInsertPosition(parent,index);parent=insertPosition.parent;const isRootContainer=self.isRootContainer(parent);const elmType=!isRootContainer?self.getElmType(parent):'';const content=$ush.toString(self.pageData.content);var oldShortcode=!isRootContainer?self.getElmShortcode(parent):content;oldShortcode=self.removeHtmlWrap(oldShortcode);var position=insertPosition.position;if(isRootContainer){position=(position==='before'||position==='after')?'container:prepend':'container:'+position}
...
if(self.isElmSection(child)){const titleTemplate=$usb.getTextTranslation('section'),params={title_1:(titleTemplate+' 1'),title_2:(titleTemplate+' 2'),vc_column_text:self.getSpareElmId('vc_column_text'),vc_column_text_content:_getDefaultContent('vc_column_text'),vc_tta_section_1:self.getSpareElmId(child),vc_tta_section_2:self.getSpareElmId(child)};return $usb.buildString($usb.config(`template.${child}`,''),params)}else if(child==='timeline_section'){params={timeline_section_usbid1:self.getSpareElmId('us_timeline_section'),timeline_section_usbid2:self.getSpareElmId('us_timeline_section'),timeline_section_usbid3:self.getSpareElmId('us_timeline_section'),vc_column_text_usbid1:self.getSpareElmId('vc_column_text'),vc_column_text_usbid2:self.getSpareElmId('vc_column_text'),vc_column_text_usbid3:self.getSpareElmId('vc_column_text'),us_text_usbid1:self.getSpareElmId('us_text'),us_text_usbid2:self.getSpareElmId('us_text'),us_text_usbid3:self.getSpareElmId('us_text'),vc_column_text_content:_getDefaultContent('vc_column_text'),};return $usb.buildString($usb.config(`template.timeline`,''),params)}else{return'['+child+' usbid="'+self.getSpareElmId(child)+'"][/'+child+']'}},addElmToPreview:function(elmId,elmIndex,parentId,callback,newTargetId){const self=this;if(!self.isValidId(elmId)){return}
...
if(typeof callback==='function'){callback.call(self,elmId)}}})},reloadElmInPreview:function(elmId,callback){const self=this;if(!self.isValidId(elmId)||self.isRootContainer(elmId)){return}
...
self.reloadElementsMap();return elmId},moveElm:function(moveId,newParentId,newIndex){const self=this;const args=arguments;if(self.isRootContainer(moveId)){$usb.log('Error: Cannot move the container',args);return!1}
const isRootContainer=self.isRootContainer(newParentId);if(self.hasSameTypeParent(moveId,newParentId)){$usb.log('Error: It is forbidden to add inside the same type',args);return}
if(!self.isValidId(moveId)||!(self.isValidId(newParentId)||isRootContainer)){$usb.log('Error: Invalid ID specified',args);return!1}
...
if(selectedElmId&&(removeId==selectedElmId||children.includes(selectedElmId))){$usb.trigger('panel.showElementsSection')}
children.push(removeId);for(const k in children){elementsMap.delete(children[k])}
$usb.navigator.removeElm(removeId);return!0},updateIdsInContent:function(content,html){const self=this;var firstElmId,customPrefix=$usb.config('designOptions.customPrefix','usb_custom_');html=$ush.toString(html);content=$ush.toString(content);content=content.replace(REGEXP_USBID,(match,input,elmId)=>{const newElmId=self.getSpareElmId(elmId);if(!firstElmId){firstElmId=newElmId}
...
$usb.panel.switchSaveButton(!0,!0);$usb.builder.savePageData(()=>{$usb.panel.switchSaveButton(!1)})},_contentChange:function(){$usb.panel.switchSaveButton($usb.builder.isPageChanged())},isVisible:function(){return this.$generalTabs.is(':visible')},showGeneralTabs:function(){const self=this;$usb.urlManager.removeParam('active').push();if(self.isVisible()){return}
$usb.trigger('panel.clearBody');$usb.navigator.resetActive();$usb.panel.clearBody();$usb.postMessage('doAction','hideHighlight');$usb.builder.setMode('editor');self.$generalTabs.removeClass('hidden');$usb.builder.selectedElmId=null},hideGeneralTabs:function(){const self=this;if(!$usb.panel.isReady()){return}
self.$addElementsButton.removeClass('active');self.$generalTabs.addClass('hidden');$usb.builder.destroyDragDrop();$usb.postMessage('builderPanel.tabSwitched','')},clickAddElements:function(){this.showElementsSection()},showElementsSection:function(){const self=this;if(!self.isVisible()){self.showGeneralTabs()}
$usb.panel.setTitle(self.$addElementsButton.attr('title'));$ush.timeout(()=>{self.$searchField[0].focus()},10);self._events.onSwitchTabs({currentTarget:self.$currentTabButton[0]});$usb.postMessage('builderPanel.tabSwitched','elements');$usb.builder.initDragDrop()},_urlManager:function(state){const self=this;const setParams=state.setParams;if(!$usb.panel.isReady()){return}
if(!setParams.active&&!self.isVisible()&&$.isEmptyObject(state.oldParams)&&!$usb.find('builder.selectedElmId')&&setParams.action!=$usb.config('actions.site_settings')){self.showElementsSection()}else if(setParams.active=='paste_row'){self.showImportContent()}},});$.extend(prototype,{showImportContent:function(){const self=this;self.clearBody();$usb.builder.setMode('editor');$usb.navigator.resetActive();$usb.panel.clearBody();self.$importContent.removeClass('hidden');self.$importTextarea.val('').removeClass('validate_error');self.$importTextarea[0].focus();self.$actionSaveImportContent.prop('disabled',!0).addClass('disabled');$usb.panel.setTitle('paste_row',!0);$usb.builder.selectedElmId=null},_showImportContent:function(){$usb.urlManager.setParam('active','paste_row').push()},_hideImportContent:function(){if($usb.panel.isReady()){this.$importContent.addClass('hidden')}},_changeImportContent:function(e){const self=this;$usb.notify.closeAll();var target=e.target,pastedContent=target.value.trim();if(pastedContent.indexOf('usbid=')!==-1){pastedContent=pastedContent.replace(USBID_ATTR_REGEXP,'')}
...
pastedContent=$usb.builder.removeHtmlWrap(pastedContent);const isValid=!(!/^\[vc_row([\s\S]*)\/vc_row\]$/gim.test(pastedContent)||pastedContent.indexOf('[vc_column')===-1);if($usb.builder.isRemoveRows){pastedContent=$usb.builder.removeRows(pastedContent)}
$textarea.toggleClass('validate_error',!isValid);if(!isValid){$usb.notify.add($usb.getTextTranslation('invalid_data'),NOTIFY_TYPE.ERROR);return}
...
$usb.history.commitChange(elmId,ACTION_CONTENT.CREATE);$usb.builder.pageData.content+=(res.data.content||pastedContent.replace(/(grid_layout_data="([^"]+)")/g,'items_layout=""'));$usb.builder.reloadElementsMap();$usb.postMessage('insertElm',[$usb.builder.rootContainer,'append',res.data.html,!0]);$usb.trigger('builder.contentChange')},complete:(_,textStatus)=>{const isSuccess=(textStatus==='success');$saveButton.prop('disabled',isSuccess).removeClass('loading').toggleClass('disabled',isSuccess);$textarea.prop('disabled',!1).removeClass('disabled');if(isSuccess){$textarea.val('')}}})}});$.extend(prototype,{_loadDeferredFieldsets:function(name){const self=this;const data={};$usb.$panel.addClass('data_loading');var requestId='loadDeferredFieldsets';if(!$ush.isUndefined(name)){data.name=name;requestId+='.name';$usb.$panel.addClass('show_preloader')}
...
if(isUpdated&&_currentData.elmType.indexOf('vc_row')===0&&['columns','columns_layout'].includes(_currentData.name)&&$usb.builder._updateColumnsLayout(_currentData.id,_currentData.value)){const columnIds=$usb.builder.getElmChildren(_currentData.id);$usb.postMessage('showPreloader',_currentData.id);$usb.builder.renderShortcode('renderShortcode',{data:{content:$usb.builder.getElmShortcode(_currentData.id),},success:(res)=>{if(res.success){$usb.postMessage('updateSelectedElm',[_currentData.id,$ush.toString(res.data.html)]);columnIds.map((theColumnId)=>{if(!$usb.builder.hasElementId(theColumnId)){$usb.postMessage('removeHtmlById',theColumnId)}})}},complete:()=>{deferred.resolve(isUpdated)}})}else{deferred.resolve(isUpdated)}
...
$usb.builder.reloadElmInPreview(elmId,()=>{$usb.builder.trigger('shortcodeChanged',_currentData)})})})}else if(instructions!==!0){const _updateOnInstructions=(_currentData)=>{_updateShortcode(_currentData).always((isUpdated)=>{if(!isUpdated||$ush.isUndefined(_currentData.instructions)){return}
if(_currentData.fieldType==='editor'&&typeof usofField.getHtmlContent==='function'){_currentData.value=usofField.getHtmlContent()}else if(_currentData.isResponsiveValue){_currentData.value=$ush.toPlainObject(_currentData.value)}
...
$.each(self.activeElmFieldset.$tabsSections,(index,section)=>{const fields=$('> *',section).toArray();var isHidden=!0;for(const k in fields){var $field=$(fields[k]),isVisible=$field.data('isShown');if($ush.isUndefined(isVisible)){isVisible=($field.css('display')!='none')}
if(isVisible){isHidden=!1;break}}
...
$.extend(Navigator.prototype,$ush.mixinEvents,{isReady:function(){return!$ush.isUndefined(this.$container)},isVisible:function(){return $usb.urlManager.hasParam('navigator','show')},_switch:function(){const urlManager=$usb.urlManager;if(!this.isVisible()){urlManager.setParam('navigator','show')}else{urlManager.removeParam('navigator')}
...
self.$container.addClass('show');self.$actionSwitch.addClass('active');self.redraw()},hide:function(){this.$container.removeClass('show');this.$actionSwitch.removeClass('active')},_scrollTo:function(id){const self=this;if(!self.isVisible()||!$usb.builder.isValidId(id)){return}
...
var headerHeight=$('.usb-navigator-header',self.$container).height();$body[0].scrollTo(0,rect.top+$body.scrollTop()-headerHeight)},_showPreloader:function(id){const self=this;if(!self.isVisible()||!$usb.builder.doesElmExist(id)){return}
...
this.trigger('showPreloader',id);$usb.trigger('builder.elmDuplicate',id);return!1},_removeElm:function(e){const id=$(e.target).closest('[data-for]').data('for');if($usb.builder.doesElmExist(id)){$usb.trigger('builder.elmDelete',id)}},removeElm:function(id){const self=this;if(!self.isVisible()||!$usb.builder.isValidId(id)){return}
$(`[data-for="${id}"]:first`,self.$body).remove()},setActive:function(id,expandParents){const self=this;if(!self.isVisible()||!$usb.builder.doesElmExist(id)){return}
self.resetActive();$(`[data-for="${id}"]`,self.$body).addClass('active').parents('[data-for]').toggleClass('expand',!!expandParents)},resetActive:function(){const self=this;if(self.isVisible()){$('[data-for].active',self.$body).removeClass('active')}},buttonControl:function(isDisabled){this.$actionSwitch.toggleClass('disabled',isDisabled)},redraw:function(){const self=this;if($usb.iframeIsReady!==!0||$usb.builder.isEmptyContent()||!self.isVisible()){return}
...
self.$body.html(getItems($usb.builder.getElmChildren($usb.builder.rootContainer),new DocumentFragment,0));self.setActive($usb.builder.selectedElmId,!0)},_panelShowMessage:function(){this.resetActive()},_shortcodeChanged:function(data){const self=this;if(!$.isPlainObject(data)){return}
if(self.isVisible()&&data.name==='el_id'){if(data.value){data.value='#'+data.value}
...
self.buttonControl(isEmptyContent);if(!isEmptyContent&&self.isVisible()){$ush.debounce_fn_1ms(self.redraw.bind(self))}},_iframeReady:function(){const self=this;const isEmptyContent=$usb.builder.isEmptyContent();self.buttonControl(isEmptyContent);if(!isEmptyContent){self._urlManager($usb.urlManager.getDataOfChange())}
if(self.isVisible()){$ush.timeout(self.redraw.bind(self),1)}},_urlManager:function(state){const self=this;if(!self.isReady()){return}
if(self.isVisible()&&state.setParams.action===$usb.config('actions.site_settings')){self.hide();return}
...
_window.$ush=_window.$ush||{};_window.$usbcore=_window.$usbcore||{};var _$tmp={$categorySections:{},isLoaded:{},};function Templates(){const self=this;self.name='templates';self._events={showTemplatesSection:self.showTemplatesSection.bind(self),expandCategory:self._expandCategory.bind(self),}
$(()=>{self.$container=$('#usb-templates');self.$currentTabButton=$('.usb_action.show_templates',$usb.$panel);self.$error=$('.usb-templates-error',self.$container);$usb.$panel.on('click','.usb-template-title',self._events.expandCategory).on('click','.usb_action.show_templates',self._events.showTemplatesSection)});$usb.on('templates.showTemplatesSection',()=>self.$currentTabButton.trigger('click'))}
$.extend(Templates.prototype,$ush.mixinEvents,{isReady:function(){return!$ush.isUndefined(this.$container)},isVisible:function(){return this.$container.is(':visible')},isTemplate:function(id){const self=this;if($usb.builder.isValidId(id)){id=$usb.builder.getElmType(id)}
return id==='import_template'},configIsLoaded:function(){return!$.isEmptyObject(_$tmp.$categorySections)},categoryIsLoaded:function(categoryId){return _$tmp.isLoaded.hasOwnProperty(categoryId)},templateIsLoaded:function(categoryId){return this.categoryIsLoaded(categoryId)&&_$tmp.isLoaded[categoryId]},showTemplatesSection:function(){const self=this;if(!$usb.builderPanel.isVisible()){$usb.builderPanel.showGeneralTabs()}
if(!self.configIsLoaded()){self._loadConfig()}
if($usb.licenseIsActivated()){$usb.builder.initDragDrop()}
if($usb.find('builderPanel')){$usb.builderPanel.$addElementsButton.addClass('active')}
$('.usb-template',self.$container).removeClass('expand');$usb.postMessage('builderPanel.tabSwitched','templates')},_expandCategory:function(e){const self=this;const categoryId=$(e.currentTarget).parent().data('template-category-id');if(!categoryId){return}
...
if(!$usb.builder.isRootContainer(parentId)){$usb.log('Error: Invalid parent container, templates can only be added to rootContainer',args);return}
...
return}
_getTemplateData()}});$usb.templates=new Templates()}(jQuery);!function($,_undefined){var _$tmp={deleteSectionId:0,listIsLoaded:!1,sectionContent:'',};function Favorites(){const self=this;self._events={changeSectionName:self._changeSectionName.bind(self),clickTabFavorites:self._clickTabFavorites.bind(self),deleteSection:self._deleteSection.bind(self),reorderSections:$ush.debounce(self._reorderSections.bind(self),1),resetSearch:self._resetSearch.bind(self),saveToFavorites:self._saveToFavorites.bind(self),saveToFavoritesByPressEnter:self._saveToFavoritesByPressEnter.bind(self),search:self._search.bind(self),setExampleValueToSectionName:self._setExampleValueToSectionName.bind(self),showConfirmDelete:self._showConfirmDelete.bind(self),showList:self.showList.bind(self),showPopupToGetName:self._showPopupToGetName.bind(self),showFavoritesSection:self.showFavoritesSection.bind(self),};$(()=>{self.$container=$('#usb-favorites');self.$search=$('.usb-panel-search',self.$container);self.$searchField=$('input[name=search]',self.$container);self.$searchNoResult=$('.usb-panel-search-noresult',self.$container);self.$list=$('.usb-favorites-list',self.$container);self.$emptyList=$('.usb-favorites-empty-list',self.$container);self.$confirmDeletion=$('.usb-favorites-confirm-deletion',self.$container);self.$currentTabButton=$('.usb_action.show_favorites',$usb.$panel);self.$container.on('input','input[name=search]',$ush.debounce(self._events.search,1)).on('click','.usb_action_reset_search_in_panel',self._events.resetSearch).on('click','.usb_action_show_confirm_delete',self._events.showConfirmDelete).on('click','.usb_action_delete_from_favorites',self._events.deleteSection).on('click','.usb_action_cancel_deletion_from_favorites',self._events.showList);$usb.$panel.on('click','.usb_action.show_favorites',self._events.clickTabFavorites);const dragDrop=new $usof.dragDrop(self.$list,'.usb-favorites-item',!0);dragDrop.on('changed',self._events.reorderSections);self.popup=new $usof.popup('popup_save_to_favorites',{closeOnEsc:!0,closeOnBgClick:!0,init:function(){const $popup=this.$container;self.$inputSectionName=$('input[name=section_name]',$popup);self.$saveButton=$('.usb_action_save_to_favorites',$popup);self.$errMessage=$('.usof-message.status_error',$popup);$($popup).on('input','input[name=section_name]',self._events.changeSectionName).on('keyup','input[name=section_name]',$ush.debounce(self._events.saveToFavoritesByPressEnter,1)).on('click','.usb_action_save_to_favorites',self._events.saveToFavorites).on('click','.usof-example',self._events.setExampleValueToSectionName)},afterShow:()=>{$ush.timeout(()=>{self.$inputSectionName[0].focus()},20)},afterHide:()=>{self.$inputSectionName.removeClass('is_invalid').val('');_$tmp.sectionContent=''}})});$usb.on('favorites.saveToFavorites',self._events.showPopupToGetName).on('favorites.showFavoritesSection',self._events.showFavoritesSection)}
$.extend(Favorites.prototype,$ush.mixinEvents,{isReady:function(){return!$ush.isUndefined(this.$container)},isVisible:function(){return this.$container.is(':visible')},showFavoritesSection:function(){const self=this;if(!$usb.builderPanel.isVisible()){$usb.builderPanel.showGeneralTabs()}
self.$currentTabButton.trigger('click')},isFavoriteSection:function(id){if($usb.builder.isValidId(id)){id=$usb.builder.getElmType(id)}
...
const newData=$usb.builder.updateIdsInContent(res.data.content,res.data.html);if(!$usb.builder.insertShortcodeIntoContent(parentId,currentIndex,newData.content)){return!1}
$usb.postMessage('insertElm',[insert.parent,insert.position,newData.html]);if($usb.builder.isRow(newData.firstElmId)){$usb.history.commitChange(newData.firstElmId,ACTION_CONTENT.CREATE)}
$ush.timeout(()=>{$usb.builder.reloadElementsMap()},0);$usb.trigger('builder.contentChange')}})},});$usb.favorites=new Favorites()}(jQuery);!function($,undefined){const _window=window;if(!_window.$usb){return}
...
$usb.builder.pageData.customCss=customCss;$usb.postMessage('updatePageCustomCss',customCss);$ush.debounce_fn_1ms(()=>$usb.trigger('builder.contentChange'));self.$actionShowCustomCss.toggleClass('css_not_empty',!!customCss)};if(!_$tmp.isInputCustomCss){$usb.history.commitData($usb.builder.pageData.customCss,setPageCustomCss);_$tmp.isInputCustomCss=!0}else{self.__debounce_2s(()=>{_$tmp.isInputCustomCss=!1})}
...
var type=task.type;delete task.type;if(type===HISTORY_TYPE.UNDO){_changesHistory.redo.push(task)}else{_changesHistory.undo.push(task)}
$ush.timeout(()=>$usb.builder.reloadElementsMap(),1)},_applyChangesFromTask:function(task,originalTask){const self=this;if($.isEmptyObject(task)){_$tmp.isActiveRecoveryTask=!1;return}
...
if(state.setParams.action===_siteSettings){var groupId=state.setParams.group;if(groupId&&self.selectedGroupId!==groupId){if(!$usb.iframeIsReady){$usb.panel.showPreloader();$usb.one('iframeReady',self.initFieldset.bind(self,groupId))}else{self.initFieldset(groupId)}}else if(!groupId){self.showGeneralMenu()}
self.$actionGoToBack.toggleClass('disabled',!state.setParams.group)}else if(state.oldParams.action===_siteSettings){self._handlerClearBody();_$tmp.savedLiveOptions={};$usb.urlManager.removeParam('group').push()}},_confirmExit:function(e){const message=$usb.getTextTranslation('page_leave_warning');if(this.isChanged()&&!confirm(message)){e.preventDefault();return}},_iframeReady:function(){const self=this;if(!$usb.iframeIsReady||!$usb.urlManager.hasParam('action',$usb.config('actions.site_settings'))){return}
