...
class WP_Import extends WP_Importer {
	...
	function import_options() {
		...
		?>
<form action="<?= admin_url( 'admin.php?import=wordpress&amp;step=2' ) ?>" method="post">
	<?php wp_nonce_field( 'import-wordpress' ) ?>
	<input type="hidden" name="import_id" value="<?= $this->id ?>" />
...
<?php endif ?>
	...
	function process_menu_item( $item ) {
		...
		if ( $menu_item_db_id AND ! is_wp_error( $menu_item_db_id ) ) {
			// Set param "Show as Button"
			if ( isset( $_menu_item_btn_style ) ) {
				update_post_meta( $menu_item_db_id, '_menu_item_btn_style', (int)$_menu_item_btn_style );
			}
			$this->processed_menu_items[ (int)$item['post_id'] ] = (int)$menu_item_db_id;
		}
	}
